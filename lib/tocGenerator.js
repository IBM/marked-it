...
module.exports = function(callbacks, urlPrefix, tocDepth, xmlMode) {
	...
	var stack = [dom]; /* a basic stack to represents TOC tree structure */
	var errors;
	...
	return {
		heading: function(text, level, attributes) {
			...
			if (!parent) {
				/*
				 * A parent ancestory TOC entry was not generated for a header (either NOTOC was specified for it or
				 * the level was too deep.
				 */
			...
			var newElement = null;
			if (!attributes[NOTOC] && level <= tocDepth) {
				var topicAttributes = [];
				if (attributes) {
				...
				newElement = callbacks.createTopic(urlPrefix + "#" + attributes.id, text, level, topicAttributes);
