...
//				topicString += label + "</a></h" + level + "></div>\n";
				...
				/* TODO should just get the level from heading, then don't need to pass it to all of these TOC generators */
				return common.invokeExtensions(
					extensions,
					"html.toc.onTopic",
	...
	if (xmlTocGenerator) {
		var xmlResult = common.invokeExtensions(
			extensions,
			"xml.toc.onComplete",
			xmlTocGenerator.buffer(),
			{
				title: title,
